...
fn simple_eval_(
    ...
    let trace_nodes = std::env::var("FASTSURFER_NATIVE_TRACE_NODES")
        .map(|value| {
            let normalized = value.trim().to_ascii_lowercase();
            normalized == "1" || normalized == "true" || normalized == "yes" || normalized == "on"
        })
        .unwrap_or(false);
    let trace_node_ms = std::env::var("FASTSURFER_NATIVE_TRACE_NODE_MS")
        .ok()
    ...
    for (node_ix, node) in graph.node.iter().enumerate() {
        let node_started = Instant::now();
        if trace_nodes {
            eprintln!(
                "[trace][candle-node] start idx={} name='{}' op='{}' inputs={} outputs={}",
                node_ix,
                node.name,
                node.op_type,
                node.input.len(),
        ...
        match node.op_type.as_str() {
            ...
            "ScatterND" => {
                ...
                if matches!(reduction, "none" | "add")
                    && data.dtype() == DType::F32
                    && updates.dtype() == DType::F32
                {
                    if let Some(output) = scatternd_fast_f32(data, &indices, updates, reduction)? {
                        values.insert(node.output[0].clone(), output);
                        continue;
                    }
                }
        ...
        let elapsed_ms = node_started.elapsed().as_millis();
        if trace_nodes && elapsed_ms >= trace_node_ms {
            eprintln!(
                "[trace][candle-node] end idx={} name='{}' op='{}' elapsed_ms={} values={}",
                node_ix,
                node.name,
                node.op_type,
                elapsed_ms,
...
fn scatternd_fast_f32(
    ...
    let Some(&k) = indices_shape.last() else {
        return Ok(None);
    };
    if k > data.rank() {
        bail!("ScatterND expects k (indices.shape[-1]) to be at most the rank of data");
    ...
    let slice_size = update_element_shape.iter().product::<usize>().max(1);
    ...
    let mut output_vec = data.flatten_all()?.to_vec1::<f32>()?;
    let flat_updates = updates.reshape(num_updates * slice_size)?.to_vec1::<f32>()?;
    ...
    for i in 0..num_updates {
        ...
        let src_base = i * slice_size;
        for j in 0..slice_size {
            let dst = flat_idx + j;
            let src = flat_updates[src_base + j];
            if reduction == "add" {
                output_vec[dst] += src;
            } else {
                output_vec[dst] = src;
    ...
    let output = Tensor::from_vec(output_vec, data_shape.to_vec(), data.device())?;
    Ok(Some(output))
}

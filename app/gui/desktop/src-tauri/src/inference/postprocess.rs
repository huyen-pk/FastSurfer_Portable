...
fn connected_components_for_mask(mask: &[u8], shape: [usize; 3]) -> Vec<Vec<usize>> {
    let [sx, sy, sz] = shape;
                    ...
                    for dx in -1isize..=1 {
                        for dy in -1isize..=1 {
                            for dz in -1isize..=1 {
                                ...
                                let nx = x0 as isize + dx;
                                let ny = y0 as isize + dy;
                                let nz = z0 as isize + dz;
                                if nx < 0
                                    || ny < 0
                                    || nz < 0
                                    || nx >= sx as isize
...
fn centroid_of_indices(indices: &[usize], shape: [usize; 3]) -> Option<[f64; 3]> {
    if indices.is_empty() {
        return None;
    }
...
pub(crate) fn split_cortex_labels(pred_labels: &mut [u16], shape_xyz: [usize; 3]) {
    let lh_wm_mask = pred_labels
        .iter()
        .map(|value| if *value == 2 { 1u8 } else { 0u8 })
        .collect::<Vec<u8>>();
    ...
    for label_current in CORTEX_SPLIT_LABELS {
        let mask = pred_labels
            .iter()
            .map(|value| if *value == label_current { 1u8 } else { 0u8 })
            .collect::<Vec<u8>>();
        ...
        for component in components {
            if let Some(centroid) = centroid_of_indices(&component, shape_xyz) {
                let dist_lh = squared_distance(centroid, lh_centroid);
                let dist_rh = squared_distance(centroid, rh_centroid);
                if dist_rh < dist_lh {
    ...
    for prob_class_lh in PROBLEMATIC_SPLIT_LABELS {
        ...
        for (idx, value) in pred_labels.iter_mut().enumerate() {
            if *value == prob_class_lh || *value == prob_class_rh {
                ...
                let dist_lh = squared_distance(xyz, lh_centroid);
                let dist_rh = squared_distance(xyz, rh_centroid);
                *value = if dist_rh < dist_lh {
                    prob_class_rh
                } else {
                    prob_class_lh
                };
...
pub(crate) fn flip_wm_islands(aseg_labels: &mut [u16], shape_xyz: [usize; 3]) {
    let [lh_wm, lh_gm, rh_wm, rh_gm] = [2u16, 3u16, 41u16, 42u16];

    let lh_wm_mask = aseg_labels
        .iter()
        .map(|value| if *value == lh_wm { 1u8 } else { 0u8 })
        .collect::<Vec<u8>>();
    let rh_wm_mask = aseg_labels
        .iter()
        .map(|value| if *value == rh_wm { 1u8 } else { 0u8 })
        .collect::<Vec<u8>>();
    ...
    for component in rh_components {
        ...
        for idx in component {
            let xyz = index_to_xyz(idx, shape_xyz);
            if squared_distance(xyz, lh_centroid) < squared_distance(xyz, rh_centroid) {
                aseg_labels[idx] = lh_wm;
            }
        }
    }
    ...
    for component in lh_components {
        ...
        for idx in component {
            let xyz = index_to_xyz(idx, shape_xyz);
            if squared_distance(xyz, rh_centroid) < squared_distance(xyz, lh_centroid) {
                aseg_labels[idx] = rh_wm;
            }
        }
    }
}
...
pub(crate) fn derive_brainmask_from_pred(pred_labels: &[u16], shape_xyz: [usize; 3]) -> Vec<u8> {
    let mut initial = pred_labels
        .iter()
        .map(|value| {
            if *value > 0 && !is_frontal_special(*value) {
                1u8
            } else {
                0u8
            }
...
pub(crate) fn mask_aseg_with_brainmask(aseg_labels: &mut [u16], brainmask: &[u8]) {
